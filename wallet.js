...
// Получить баланс продавца (с холдом)
export async function getPartnerWalletBalance(partnerId) {
    try {
        ...
        // Создаём баланс по умолчанию для партнёра
        const defaultBalance = {
            userId: partnerId,
            userType: "partner",
            available: 0,
            pending: 0,         // Замороженные средства (холд)
            totalEarned: 0,
            totalWithdrawn: 0,
            updatedAt: new Date()
        };
        await setDoc(doc(db, "wallet_balances", partnerId), defaultBalance);
        return defaultBalance;
    } catch (err) {
        console.error("Ошибка получения баланса партнёра:", err);
...
export async function releaseFunds(buyerId, sellerId, totalAmount, orderId) {
    ...
    const sellerAmount = Math.round(totalAmount * 0.9 * 100) / 100;
    ...
    try {
        // 1. Снять холд с покупателя
        const buyerDoc = await getDoc(buyerRef);
        const buyerBalance = buyerDoc.exists() ? buyerDoc.data() : { pending: 0, totalSpent: 0 };
        
        await updateDoc(buyerRef, {
            pending: Math.max(0, (buyerBalance.pending || 0) - totalAmount),
            totalSpent: (buyerBalance.totalSpent || 0) + totalAmount,
            updatedAt: new Date()
        });
        
        // 2. Зачислить продавцу (используем существующую функцию)
        await addToPartnerWallet(sellerId, sellerAmount, orderId, `Продажа по заказу ${orderId.slice(0, 8)} (90%)`);
        
        // 3. Зачислить комиссию платформе
        const prorankDoc = await getDoc(prorankRef);
        const prorankBalance = prorankDoc.exists() ? prorankDoc.data() : { available: 0, totalEarned: 0 };
        
        await updateDoc(prorankRef, {
            available: (prorankBalance.available || 0) + commissionAmount,
            totalEarned: (prorankBalance.totalEarned || 0) + commissionAmount,
            updatedAt: new Date()
        });
        
        // 4. Транзакция для комиссии
        await addDoc(collection(db, "wallet_transactions"), {
